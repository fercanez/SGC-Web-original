...
def get_catalog_summary(
    ...
    """Conteos de catálogos reales del sistema catastral."""
    row = db.execute(
        text("""
            SELECT
                (SELECT COUNT(*) FROM catastro.predios) AS predios,
                (SELECT COUNT(*) FROM catalogos.padron_2026) AS padron,
                (SELECT COUNT(*) FROM catalogos.personas) AS personas,
                (SELECT COUNT(*) FROM catastro.predio_propietario) AS predio_propietario,
                (SELECT COUNT(*) FROM catalogos.cat_colonias) AS colonias,
                (SELECT COUNT(*) FROM catalogos.cat_calles) AS calles,
                (SELECT COUNT(*) FROM catalogos.cat_usos_prediales) AS usos_prediales,
                (SELECT COUNT(*) FROM catalogos.cat_zonas_homogeneas) AS zonas_homogeneas
        """)
    ).mappings().first()
