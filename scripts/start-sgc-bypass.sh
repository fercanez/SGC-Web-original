#!/bin/sh
# Arranca SGC-Web con docker run (evita bug ContainerConfig de docker-compose 1.29)
# Uso: cd /opt/sgc-web && sh scripts/start-sgc-bypass.sh

set -e
cd /opt/sgc-web

# Lee una variable de .env sin hacer "source" (evita fallo con "Baja California", CRLF, etc.)
env_val() {
  key=$1
  default=$2
  if [ -f .env ]; then
...
docker ps -a --format '{{.Names}}' | grep -E 'sgc-web|_sgc-web' | while read -r n; do
...
echo ""
echo "=== Variables GeoNode en API ==="
docker exec sgc-web_api_1 printenv GEONODE_WMS_LAYERS GEONODE_SOURCE_LAYER 2>/dev/null || true

echo ""
echo "=== Config (capas WMS) ==="
curl -sf http://127.0.0.1:8000/api/v1/config | head -c 600
echo ""

echo ""
curl -sf http://127.0.0.1:8000/api/v1/health && echo "  OK api:8000" || echo "  FALLO api:8000"
curl -sf http://127.0.0.1:5173/api/v1/health && echo "  OK proxy:5173" || echo "  FALLO proxy:5173"
curl -sf http://127.0.0.1:8000/api/v1/geonode/status && echo ""
